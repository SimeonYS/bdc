...
class BdcPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bdc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bdc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bdc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bdc (date, title, link, content)"
